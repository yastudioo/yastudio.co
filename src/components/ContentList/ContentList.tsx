...
export const ContentList: React.FC<ContentListItemProps> = ({
  ...
  const SkillType = node.frontmatter.type
  const featuredImgFluid = node.frontmatter.featuredImage.childImageSharp.fluid
  const SkillLink = SkillType === 'post' ? node.fields.slug : node.frontmatter.url
  return (
    <Tooltip
      position="top"
      followCursor={true}
    ...
    >
      <ItemContainer>
        <ItemHeader>
          <ItemTitle to={SkillLink}>{title}</ItemTitle>
          <ItemDescription to={SkillLink}>
            {node.frontmatter.description}
          </ItemDescription>
          <ItemTags>
            {type && (
              <TypeTag
                to={`/${SkillType === 'project' ? 'projects' : 'blog'}`}
                key={SkillType}
              >
                {SkillType}
              </TypeTag>
